...
public class FakeDB {
    ...
    //returns an array of hard coded ministry teams
    public JsonArray ministryTeamsJson() {
        JsonParser parser = new JsonParser();
        String ministryTeams =
        ...
        return parser.parse(ministryTeams).getAsJsonArray();
    }

    private MinistryTeam makeTeam(JsonObject mt) {
    ...
    //returns a collection of hard coded MinistryTeams as  MinistryTeams objects
    public ArrayList<MinistryTeam> getMinistryTeams() {
        JsonArray jArr = ministryTeamsJson();
        Iterator<JsonElement> it = jArr.iterator();
        ArrayList<MinistryTeam> toReturn = new ArrayList<MinistryTeam>();

        while(it.hasNext()) {
            toReturn.add(makeTeam(it.next().getAsJsonObject()));
        }
